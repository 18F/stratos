...
export class CfUserPermissionsChecker extends BaseCurrentUserPermissionsChecker implements ICurrentUserPermissionsChecker {
  ...
  check(
    ...
    // TODO: RC return null if unknown type
 ...
 */
  ...
  // getGroupType(config: PermissionConfig) {
  ...
  public getCheckFromConfig(
    ...
    permission: CfPermissionTypes,// | CHECKER_GROUPS,
    endpointGuid?: string,
    orgOrSpaceGuid?: string,
    spaceGuid?: string
  ): ICheckCombiner {
    ...
    if (checkCombiner) {
      checkCombiner.checks.map(check => this.applyAdminCheck(check, endpointGuid))
    }
  ...
  private getBaseCheckFromConfig(
    ...
    switch (permission) {
      ...
      // case CHECKER_GROUPS.CF_GROUP: //cfPermissionTypes.ORGANIZATION || config.type === cfPermissionTypes.SPACE
      case CfPermissionTypes.ORGANIZATION || CfPermissionTypes.SPACE:
        return {
